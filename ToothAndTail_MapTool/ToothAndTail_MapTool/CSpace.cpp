...
bool CSpace::IsObjInside(const CObj * rObj, OBJ::E_COLLIDED* _pCollidedPoint, float* _pCollidedDepth) const
{
	if (rObj->GetTop() < GetTop()) {
		...
		if (_pCollidedDepth) *_pCollidedDepth = static_cast<float>(rObj->GetTop() - GetTop());
		return false;
	}
	if (rObj->GetBottom() > GetBottom()) {
		if (_pCollidedPoint) *_pCollidedPoint = OBJ::COL_BOTTOM;
		if (_pCollidedDepth) *_pCollidedDepth = static_cast<float>(rObj->GetBottom() - GetBottom());
		return false;
	}
	if (rObj->GetLeft() < GetLeft()) {
		if (_pCollidedPoint) *_pCollidedPoint = OBJ::COL_LEFT;
		if (_pCollidedDepth) *_pCollidedDepth = static_cast<float>(rObj->GetLeft() - GetLeft());
		return false;
	}
	if (rObj->GetRight() > GetRight()) {
		if (_pCollidedPoint) *_pCollidedPoint = OBJ::COL_RIGHT;
		if (_pCollidedDepth) *_pCollidedDepth = static_cast<float>(rObj->GetRight() - GetRight());
		return false;
	}
